...
1. 기존 프로젝트 SQL Editor에서 [`migrations/202609200004_contest_guests.sql`](migrations/202609200004_contest_guests.sql)만 한 번 적용한다. **아직 원격 적용하지 않음**. 001~003과 회원/예제/채용 데이터는 변경하지 않는다.
2. `PROOFOLIO_STORAGE=supabase` 및 기존 서버 키를 유지한다. 대회 기본값은 다음과 같으며 루트 `.env` 또는 프로세스 환경에서 재정의할 수 있다.

```dotenv
PROOFOLIO_CONTEST_MODE=1
PROOFOLIO_CONTEST_CLOSED=0
PROOFOLIO_CONTEST_ENDS_AT=2026-10-20T23:59:59+09:00
```

3. 비회원 분석은 새 `proofolio_start_guest_run` RPC에서 회원과 같은 전역 DB 잠금을 사용한다. 24시간 1회·전체 동시 1회·6~10개·최대 24시간 보관을 확인한 뒤에만 파일을 저장하고 유료 작업을 시작한다. 삭제로 횟수 제한이 초기화되지 않으며, 계정/메일/이름/연락처를 생성하지 않는다. 쿠키 재발급을 완전한 신원 기반 제한으로 설명하지 않는다.
4. `proofolio_users.is_guest`는 이번 새 체험자만 표시한다. 기존 익명 예제 소유자는 그대로 두고, 회원을 익명으로 변환하지 않는다. 쿠키 원문은 DB/응답에 저장하지 않는다.
5. 만료되면 새 API 접근을 막고, 정리 작업이 15분마다 최대 20개를 Storage → 해당 로컬 실행 폴더 → DB 순서로 삭제한다. 로컬 삭제 실패 시에도 DB 행을 재시도 표식으로 남긴다. 이미 발급된 원문 signed URL은 최대 60초간 유효하다. 마지막 실행 후 24시간이 지나고 실행/채용 참조가 없는 체험 프로필도 정리한다. 미제출 브라우저 초안은 기존 24시간 만료를 유지하며 재방문 시 만료를 확인해 제거한다(오프라인 브라우저의 물리 삭제는 보장하지 않음).
6. 날짜 종료 또는 `PROOFOLIO_CONTEST_CLOSED=1` + 재시작 시 모든 체험 경로를 닫고 **새 guest 표시 데이터만** 만료시킨다. 기존 회원/예제 데이터는 이 종료 처리로 삭제하지 않는다. 진행 중 작업은 최대 2시간 제한 후 정리한다. 서버를 먼저 끄지 말고 활성 작업 종료와 정리 성공을 확인한 후 종료한다. 로컬/Storage 삭제 실패는 자동으로 성공 처리하지 않는다.

`tests/verify_contest.sql`은 격리 DB에서 실행하는 롤백 검사다. 로컬 PostgreSQL WASM(PGlite)로 001~004와 4개 검증 SQL 통과. 이 결과는 원격 Supabase 적용/실제 모델 E2E 성공을 뜻하지 않는다. 원격 SQL/유료 분석은 별도 단계다.

아래는 보존된 회원 모드의 설정과 과거 확인 기록이다.

2026-09-20 대상 프로젝트에 migration **001~006 적용**. 이메일·비밀번호 로그인만 사용한다. **Google 로그인·플랫폼 전체 관리자는 제외**. `feat/testscore`는 [migration **007**](migrations/202609200007_submission_scores.sql)(`proofolio_submission_scores`, `proofolio_save_submission_score`, `proofolio_test_summaries`에 `scored_count`/`average_score`)를 추가하며 대상 프로젝트에 별도 적용이 필요하다. 기존 004~006을 다시 실행하지 않는다. 이 SQL은 채점 결과 저장용이며 Vercel 채점 실행·공유 예산을 활성화하지 않는다. 실제 메일 발송 설정/수신 확인은 사용자 요청으로 보류했으며 메일 확인을 끄지는 않았다.
...
2026-09-20 추가 요청으로 아래 시딩을 다시 실행해 동일 데이터 재사용을 확인했다. Storage **디자인 10개 + 마케팅 10개 + 코딩 9개 = 29개**(각 analysis.json 포함)를 실제 다운로드 검사했다. 디자인/마케팅 이미지 각8개, 총16개가 있으며 PNG 형식과 비어 있지 않음을 확인했다. 원본·질문·비용 기록을 덮어쓰거나 새 AI 분석을 하지 않았다. 공개 `/dashboard`는 이 예제 질문만 조회한다. PDF 출처 기록이 재배포 불가이므로 **비공개 저장과 데모 이미지 공개는 별개**이며 공개 허락 확인 전에는 원문 URL을 내주지 않는다.
...
남은 운영 작업은 플랫폼 전체 관리자 권한/화면(필요 시), 사용자가 보류한 메일 설정·수신 확인, 운영 예산 연결이다. 음성·Google 로그인은 이번 범위가 아니다. AI 답변 채점은 007 적용 후 서버 키로만 저장·조회하며, Vercel 채점 실행은 DB 예산·중복 실행 제어 연결 전까지 차단한다. 현재 공개 프리뷰에는 채점을 연결하지 않았다.
