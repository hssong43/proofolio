export async function register() {
  ...
  const {contestSettings}=await import('./lib/server/contest.ts');
  ...
  state.proofolioCleanupTimer=setInterval(clean,contestSettings().enabled?15*60*1000:24*60*60*1000);state.proofolioCleanupTimer.unref();clean();
}
