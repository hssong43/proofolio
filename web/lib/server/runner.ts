...
// Shared with retention across Next route bundles; this service runs on one persistent Node host.
const runtimeState = globalThis as typeof globalThis & { proofolioActiveRuns?: Set<string> };
const activeRuns = runtimeState.proofolioActiveRuns ??= new Set<string>();
export const isActiveRun = (id: string) => activeRuns.has(id);
...
export async function startRun(input: { bytes: Uint8Array; fileName: string; track: Track; maxQuestions?: number; userId?: string; member?: boolean; guestExpiresAt?: string }) {
  ...
  try {
    ...
    if (existsSync(dir)) { try { await writeStatus(status); } catch { /* DB failure state remains available. */ } }
    if (input.guestExpiresAt && !admitted && e instanceof Error) {
  ...
  child.on("close", (code) => { void finish(code).catch(() => console.error("분석 상태를 저장하지 못했습니다. 로컬 디스크를 확인하세요.")).finally(() => activeRuns.delete(runId)); });
  return status;
}
