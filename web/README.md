...
- 사용자 요청으로 Supabase에 저장된 디자인4/5/24·마케팅6/8페이지 PNG를 공개 프록시 `/api/examples/:slug/image?page=N`에서 제공한다. 등록된 예제·라이브러리 소유자·질문 연결 페이지·PNG 경로를 확인하며, private 버킷/전체 PDF/실제 방문자 파일은 공개하지 않는다. 코딩은 공개 저장소 링크와 기존 코드 인용을 표시한다. 새 SQL·재업로드는 필요 없다.
