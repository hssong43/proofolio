...
**업로드 수정과 분석 배포는 별개다.** 사용자 요청으로 새 분석 서버/GCP는 추가하지 않았다.
현재 영속 디스크/장시간 자식 프로세스 분석은 Vercel에서 실행하지 않고 503으로 안내한다.
예제·기존 결과/답변·Storage 직접 업로드는 이용 가능하지만 새 질문 생성까지 배포 완료된 상태는 아니다.
상시 Node 서버에서는 기존 분석 코어와 비용 원장을 그대로 사용한다. 로컬 multipart 호출도 호환 유지한다.
...
- SQL **004**와 Supabase·승인된 유료 예산이 실제 분석의 선행 조건이다. 기본 예산 0에서는 모델을 호출하지 않는다. Vercel 빌드와 상시 분석 서버 배포는 여전히 별개다.
...
- `?demo=1`: 기존 DB 예제(디자인 7·마케팅 9·코딩 7)를 그대로 표시한다. 새 모델 호출·업로드·답변 DB 저장이 없다. 원본 PDF는 비공개이고, 관리자 패널에서는 질문 연결 원본 페이지를 볼 수 있다. 관리자 예시 답변은 실제 응시/답변 저장과 분리한다.
