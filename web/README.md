...
- 채용 담당자 화면은 제출별 AI 채점(종합 점수·문항별 점수·포함/누락 항목·코멘트)과 테스트별 평균을 표시한다. 채점은 제출 완료 시 서버가 백그라운드로 실행하며(`web/lib/server/scoring.ts` → `src/scoring.ts`), 실패하면 `다시 채점` 버튼으로 재실행한다. 합불 판정은 없다.
