...
- **2026-09-23 공개 예제 미디어 중단:** 디자인·마케팅 전체 PDF/이미지는 관리자 `포트폴리오 보기`, 질문·답변 카드, 예제 응시 화면 모두 **이미지 공개 불가**로 표시한다. 기존 질문·예시 답변·점수와 코딩 저장소 보기는 유지한다.
- 예제 API는 `images: []`, `portfolioUrl: null`을 반환하고 `sourceAssets`는 공개하지 않는다. 기존 `/api/examples/:slug/image`·`portfolio` 주소도 **410/no-store**로 차단하며 Storage 조회·서명 URL 발급을 하지 않는다. 화면만 숨기는 변경이 아니다. 이미 발급된 옛 PDF 서명 URL은 기존 최대5분 유효기간 이후 만료되고, 이미 받은 사본은 회수할 수 없다.
- private 원본 파일/DB 결과는 삭제하지 않는다. 실제 사용자 업로드의 비공개 이미지·PDF 보기, 분석·답변 저장 경로는 유지한다. 공개 예제는 **이미지 공개 불가 → 질문 → 답변**, 실제 질문은 **원본 이미지 → 질문 → 답변** 순서다. 원문 인용·코드 블록·추출 안내는 질문 화면에 노출하지 않는다.
...
- 검증은 유료 분석 없이 기존 예제·합성 질문으로 수행하며 최신 결과는 루트 `CHECKPOINT.md`를 따른다.
