...
- 역할 선택·업로드·분석·준비·완료·공개 예제 관리자·회원 채용 관리자 전체에서 검토 권장·검수 미지원·근거 부족·품질 제한 안내와 자동 평가 미지원 통계를 표시하지 않는다. 실제 질문 수/시간, 개인정보/저장 안내와 오류는 유지한다. 내부 결과의 `status`/`qualityIssues`와 질문 0개 차단은 변경하지 않는다.
