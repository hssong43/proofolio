...
- 기본은 `PROOFOLIO_CONTEST_MODE=1`. `/`와 `/demo`에서 로그인 없이 분석·답변 저장·브라우저별 기록을 사용한다. `?demo=1`만 기존 예제 체험이다.
- 가입/로그인/채용 코드/응시 개인정보 입력은 보이지 않으며 API도 차단한다. 운영자/담당자 정보를 공개한 것이 아니다. 기존 회원 모드는 `PROOFOLIO_CONTEST_MODE=0`으로만 실행한다.
- `/dashboard`와 상단 **관리자 데모** 링크는 로그인 없이 공개한다. 같은 `/api/examples`에서 3종 예제·23문항·인용·의도/가이드만 조회하고 실제 유저/답변·관리 API에 접근하지 않는다. 예제 답변은 만들어내지 않으며 수정/삭제/분석 실행 버튼이 없다. 이미지 16개는 Supabase에 있지만 원 출처 공개 허락을 확인하기 전에는 signed URL을 공개하지 않는다.
