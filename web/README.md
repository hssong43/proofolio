...
데스크톱/모바일 22개 검사로 문항별 저장 ACK/실패/재시도, 새로고침 복구, 이메일 화면, 코딩 입력과 API 차단을 검사한다. 키는 빈 값, 예산은 0이며 기존 서버를 재사용하지 않는다.
...
- 상시 실행 단일 Node·영속 디스크가 필요한 MVP다. 관리자 화면, 배포 도메인/메일/운영 예산 설정은 남았다. 작업 큐 없이 서버리스/다중 인스턴스로 옮기지 않는다.
