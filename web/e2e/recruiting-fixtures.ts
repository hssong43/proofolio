...
export const testRecord:TestSummary={id:testId,code:'ABC234',title:'합성 채용 테스트',role:'designer',
  ...
  createdAt:new Date().toISOString(),totalSeconds:40,questionCount:10,status:'open',submissionCount:1,completedCount:1,scoredCount:1,averageScore:62};
...
/** 6문항 합성 채점: q1 전부 포함(가산), q2~q4 일부 누락(가산 없음), q5 항목 전부 누락(답변은 있음, 60), q6 빈 답변(0). 평균 62. */
export const score:SubmissionScore={state:'complete',model:'anthropic/claude-opus-5',overallScore:62,error:null,scoredAt:new Date().toISOString(),costUsd:0.0421,
  items:result.questions.map((q,i)=>{
    ...
    const score=[100,70,70,70,60,0][i],coverageScore=[80,70,70,70,60,0][i],bonus=[20,0,0,0,0,0][i];
