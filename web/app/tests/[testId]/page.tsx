...
export default async function TestDetailPage({ params }: { params: Promise<{ testId: string }> }) {
  ...
  const roleLabel = ROLES.find((r) => r.id === test.role)?.label ?? test.role;
  return (
        ...
        <div className="dash-title-row">
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <p className="screen-subtitle" style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
              <span className="chip chip-sm">{roleLabel}</span>
              ...
              <span>통과 기준 {test.passScore}점</span>
          ...
          {submissions.length === 0 ? (
            ...
            <div className="table-wrap">
              <table className="table">
                <thead>
                  <tr><th>이름</th><th>생년월일</th><th>전화번호</th><th>상태</th><th>제출 시각</th><th>답변</th><th>점수</th><th>결과</th><th></th></tr>
                </thead>
                <tbody>
                  {submissions.map((s) => (
                    <tr key={s.id}>
                      ...
                      <td style={{ fontVariantNumeric: "tabular-nums" }}>{s.evaluation ? `${s.evaluation.overallScore}점` : "-"}</td>
                      <td>{s.evaluation ? <StatusBadge status={s.evaluation.passed ? "passed" : "failed"} /> : "-"}</td>
