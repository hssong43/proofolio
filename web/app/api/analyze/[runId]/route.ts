...
export async function DELETE(request: Request, context: { params: Promise<{ runId: string }> }) {
  ...
  try {
    ...
    await dbRequest(`/rest/v1/proofolio_runs?id=eq.${runId}&user_id=eq.${user.id}`, { method: 'PATCH',
      body: JSON.stringify({ deleted_at: new Date().toISOString(), expires_at: new Date(Date.now() + (user.member ? 7 * 86400000 : 0)).toISOString() }) });
    return NextResponse.json({ ok: true, message: user.member ? '목록에서 숨겼어요. 7일 후 원문과 기록이 정리돼요.' : '접근을 차단했어요. 다음 정리 주기에 원문과 답변을 삭제해요.' });
