...
export function AccountApp({ resumeRunId, recovery, authError, loginRequested, returnTo, contest=false, ...flowProps }: VerificationFlowProps & { recovery?: boolean; authError?: boolean; loginRequested?: boolean; returnTo?: string; contest?: boolean }) {
  ...
  const [message, setMessage] = useState(!contest && authError ? '인증 링크를 확인하지 못했어요. 다시 로그인해주세요.' : ''), [busy, setBusy] = useState(false);
  ...
  useEffect(() => {
    const start = contest ? api('/api/session', {method:'POST'}).then(() => setVisitorReady(true)) : refresh();
    void start.catch(e => setMessage(contest ? e.message : '로그인 서버 설정을 확인해주세요. 예제 체험은 이용할 수 있어요.')).finally(() => setLoading(false));
  }, [contest]);
  useEffect(() => { if (!contest && recovery && !loading) setLoginOpen(true); }, [contest, recovery, loading]);
  useEffect(() => {
    if (!contest && loginRequested && !loading) {
      if (member && returnTo) location.replace(returnTo);
      else if (!member) setLoginOpen(true);
    }
  }, [contest, loginRequested, loading, member, returnTo]);
  ...
  return <>
    <div className="account-bar">
      ...
      <div>
        ...
        </> : !contest && <button className="text-button" onClick={() => { setMode('signin'); setMessage(''); setLoginOpen(true); }}>이메일 로그인</button>}
      </div>
    </div>
    ...
    {!loading && (!contest || visitorReady || demo) && (history ? <main className="app-main"><section className="screen">
      ...
      {history.map(run => <article className="card history-card" key={run.id}>
        ...
        <div className="history-actions">
          ...
          {!['running', 'queued'].includes(run.state) && <button className="text-button" onClick={() => {
            if (!confirm(contest ? '접근을 차단하고 원문과 답변을 삭제 대상으로 지정할까요?' : '이 기록을 목록에서 삭제할까요? 7일 뒤 원문과 답변이 정리됩니다.')) return;
            void api(`/api/analyze/${run.id}`, { method: 'DELETE' }).then(() => {
              try { localStorage.removeItem(`proofolio:draft:v1:${run.id}`); } catch {}
              return showHistory();
      ...
      demo={contest ? demo : !member || demo} resumeRunId={contest || member ? runId : undefined} />)}
    {!contest && <dialog ref={dialog} className="account-dialog" onCancel={() => setLoginOpen(false)} onClose={() => setLoginOpen(false)}>
