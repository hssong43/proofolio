...
import type {Question, QuestionCard, QuestionDraft, QuestionPlan, ResolvedEvidence} from './schema.ts';
...
const ANGLES={
  ...
  process:{question:'이 자료와 관련한 작업 과정은 어떻게 설명할 수 있나요? 문서에 나타난 내용과 직접 수행한 부분을 구분하고, 변경한 부분이 있다면 그 이유를 설명해 주세요.',
    intent:'작업 과정과 변경 판단 확인',listen_for:['자료에 나타난 작업 과정','직접 수행한 부분과 그렇지 않은 부분의 구분','변경 여부와 있었다면 이유']},
  measurement:{question:'이 자료의 주장이나 결과를 어떤 기준과 조건으로 해석해야 하나요? 명시된 조건과 추가 확인할 조건을 구분하고, 실제로 검증했다면 방법과 한계를 설명해 주세요.',
    intent:'결과 해석에 필요한 조건과 검증 범위 확인',listen_for:['주장이나 결과의 정확한 의미','명시된 조건과 미확인 조건의 구분','검증 여부와 했다면 방법 및 한계']},
...
export function materializeQuestion(plan:QuestionPlan,source:ResolvedEvidence):QuestionDraft {
  const anchor=source.anchors[0],fact=anchor?.quote??anchor?.visual_description;
  if(!fact)throw new Error('질문 출처가 없습니다.');
  const body=ANGLES[plan.angle];
  return {...plan,question:(anchor.quote?'원문: ':'시각 관찰: ')+fact+'\n'+body.question,
    intent:body.intent,listen_for:[...body.listen_for],answer_target:body.intent};
}
...
export function questionQuality(cards:QuestionCard[],selectedPointIds:string[]) {
  const coverage=selectedPointIds.map(id=>({focus_target_id:id,question_ids:cards.filter(q=>q.focus_target_id===id).map(q=>q.id)}));
...
export async function generateQuestions(evidence:ResolvedEvidence[],request:Request,options:{
  selectedPointIds:string[];imagesFor:(cards:Array<{question_id:string;source:ResolvedEvidence}>)=>Promise<Array<[string,Uint8Array]>>;
}) {
  ...
  const byId=new Map(eligible.map(e=>[e.id,e]));let offset=0;
  ...
  const response=await request('QuestionSet',QuestionDrafts,{prompt:QUESTION_PROMPT+
    ...
    '\n미충족 조건: '+JSON.stringify(questionQuality(cards,options.selectedPointIds))+
  ...
  for(const [index,plan] of response.questions.entries()){
    ...
    // ponytail: one question per point/angle; favors distinct answers over extra count in a five-question MVP.
    if([...cards,...pending.map(p=>({...p.question,focus_target_id:p.source.focus_target_id,project_key:p.source.project_key}))].some(c=>
      c.focus_target_id===source.focus_target_id&&c.project_key===source.project_key&&c.angle===q.angle))errors.push('repeated_point_angle');
  ...
  if(pending.length){
    const reviewed=await request('QuestionReviews',GroundedQuestionReviews,{images:await options.imagesFor(pending),
      ...
      '\n'+JSON.stringify(pending.map(p=>({question_id:p.question_id,...p.question,source:{category:p.source.category,anchors:p.source.anchors}})))},data=>{
    ...
    for(const p of pending){
      ...
      if(accepted){const {anchor_indices,...question}=p.question;
        cards.push({id:'q'+(cards.length+1),...question,project_key:p.source.project_key,anchors:p.source.anchors,
          ...
          focus_target_id:p.source.focus_target_id,document_support:p.source.document_support});}
    }
  }
  offset+=response.questions.length;
  if(questionQuality(cards,options.selectedPointIds).status==='ready'||!response.questions.length)break;
  }
