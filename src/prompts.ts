...
같은 evidence_id는 한 번만 쓴다. 서로 다른 영역이라도 같은 답을 요구하면 중복이다. 참여 확인 질문은 최대 한 개, 나머지는 실질적인 설명 과제로 만든다.
검증된 근거가 충분할 때 3~5개를 목표로 하되 수를 채우려고 사실이나 근거를 만들지 않는다. 선정 포인트 각각의 핵심을 실제로 묻는다.
