...
- 시험은 실제 호출을 반복하지 않고 종료 기록만 보존했다. 공개 잔여 예산은 유지한다. 추가 유료 전체 시험은 별도 사용자 승인 전에는 시작하지 않는다. 원본·응답·화면·시험 스크립트는 Git 제외 `output/operations/`에만 저장한다. 후속 배포/실행 결과는 CHECKPOINT 최신 기록을 따른다.
