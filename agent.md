...
- 사용자 요청으로 디자인/마케팅 **질문 연결 원본 페이지5장**(디자인4/5/24, 마케팅6/8)을 공개 이미지 프록시 `/api/examples/:slug/image?page=N`로 제공한다. 예제 라이브러리 소유자·등록된 실행·질문 연결 페이지·정확한 PNG 경로를 모두 제한한다. 버킷은 private 그대로, 전체 PDF/크롭/실제 유저 원문/Storage 경로/키는 공개하지 않는다. 코딩은 기존 공개 저장소 링크와 저장된 코드 인용을 보여준다. 아래의 원본 이미지 공개 보류 설명은 이전 상태다. 공개 요청은 사용자에게서 받았으며 원 출처의 라이선스 메타데이터를 임의 변경하지 않았다.
- 이 변경에는 새 SQL·DB 쓰기·이미지 재업로드·유료 모델 호출이 없다. UI/접근 경계 회귀 코드는 갱신했지만 사용자 요청에 따라 **검사·빌드·브라우저 실행은 하지 않았다**. 반영 브랜치는 `codex/contest-guest-demo`이며 main 병합·배포 확인은 별도다.
